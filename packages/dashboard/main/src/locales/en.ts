export default {
  // Common
  'common.loading': 'Loading...',
  'common.save': 'Save',
  'common.cancel': 'Cancel',
  'common.delete': 'Delete',
  'common.edit': 'Edit',
  'common.create': 'Create',
  'common.add': 'Add',
  'common.search': 'Search',
  'common.filter': 'Filter',
  'common.back': 'Back',
  'common.confirm': 'Confirm',
  'common.yes': 'Yes',
  'common.no': 'No',
  'common.close': 'Close',
  'common.view': 'View',
  'common.actions': 'Actions',
  'common.status': 'Status',
  'common.active': 'Active',
  'common.inactive': 'Inactive',
  'common.enable': 'Enable',
  'common.disable': 'Disable',
  'common.retry': 'Retry',
  'common.refresh': 'Refresh',
  'common.reset': 'Reset',
  'common.submit': 'Submit',
  'common.clear': 'Clear',
  'common.default': 'Default',
  'common.all': 'All',
  'common.none': 'None',
  'common.total': 'Total',
  'common.items': 'items',
  'common.selected': 'Selected',
  'common.required': 'Required',
  'common.optional': 'Optional',
  'common.description': 'Description',
  'common.name': 'Name',
  'common.type': 'Type',
  'common.price': 'Price',
  'common.error': 'Error',
  'common.success': 'Success',
  'common.warning': 'Warning',
  'common.info': 'Info',
  'common.manage': 'Manage',

  // Navigation
  'nav.dashboard': 'Dashboard',
  'nav.shopAndItem': 'Shop & Item',
  'nav.item': 'Item Management',
  'nav.shop': 'Shop Management',
  'nav.npc': 'NPC Management',
  'nav.npcTemplates': 'Template Management',
  'nav.npcInstances': 'Instance Management',
  'nav.npcAppearances': 'Appearance Management',
  'nav.quest': 'Quest Management',
  'nav.territory': 'Territory Editor',
  'nav.systemsData': 'Systems Data',
  'nav.map': 'Map Management',
  'nav.mapEditor': 'Map Editor',
  'nav.mapAssets': 'Building Assets',
  'nav.users': 'User Management',
  'nav.userAccounts': 'Accounts',
  'nav.userGroups': 'Groups',
  'nav.settings': 'Settings',
  'nav.comingSoon': 'Coming Soon',

  // Dashboard Home
  'home.title': 'Game Management Center',
  'home.subtitle': 'Manage your game content and settings',
  'home.quickActions': 'Quick Actions',
  'home.recentActivity': 'Recent Activity',
  'home.statistics': 'Statistics',
  'home.itemManagement': 'Item Management',
  'home.itemManagement.desc': 'Manage game items, equipment and consumables',
  'home.shopManagement.desc': 'Configure shops, items, and pricing in the game',
  'home.npcManagement': 'NPC Management',
  'home.npcManagement.desc': 'Manage NPC templates and instances',
  'home.npcTemplates.desc': 'Create and manage reusable NPC templates',
  'home.npcInstances.desc': 'Deploy NPC instances in the game world',
  'home.npcAppearances.desc': 'Customize NPC appearance with 3D character models',

  // Item Management
  'item.title': 'Item Management',
  'item.subtitle': 'Manage all items in the game',
  'item.create': 'Create Item',
  'item.new': 'New Item',
  'item.newDescription': 'Create a new game item',
  'item.edit': 'Edit Item',
  'item.delete': 'Delete Item',
  'item.deleteConfirm': 'Confirm Delete',
  'item.deleteCancel': 'Cancel',
  'item.searchPlaceholder': 'Search item name or description...',
  'item.filterByCategory': 'Item Category',
  'item.filterByStatus': 'Status',
  'item.noItems': 'No items created yet',
  'item.noMatchingItems': 'No matching items found',
  'item.createFirst': 'Create your first item',
  'item.basicInfo': 'Basic Information',
  'item.economic': 'Economic Settings',
  'item.attributes': 'Attributes',
  'item.preview': 'Preview',
  'item.name': 'Item Name',
  'item.namePlaceholder': 'Enter item name',
  'item.description': 'Item Description',
  'item.descriptionPlaceholder': 'Enter item description (supports multiple lines)',
  'item.category': 'Item Category',
  'item.categoryHint': 'Different categories show different dynamic attribute settings',
  'item.category.consumable': 'Consumables',
  'item.category.special': 'Special Items',
  'item.category.equipment': 'Equipment',
  'item.category.material': 'Materials',
  'item.image': 'Item Image',
  'item.selectImage': 'Select Image',
  'item.imageHint': 'Recommended size: 128x128 or 64x64 pixels',
  'item.isActive': 'Enable this item (can appear in game)',
  'item.economicAttributes': 'Economic Attributes',
  'item.buyPrice': 'Buy Price',
  'item.buyPriceHint': 'Price for players to purchase this item',
  'item.sellPrice': 'Sell Price',
  'item.sellPriceHint': 'Price for players to sell this item',
  'item.isTradeable': 'Tradeable',
  'item.isTradeableHint': 'Allow players to trade this item',
  'item.isDroppable': 'Droppable',
  'item.isDroppableHint': 'Allow players to drop this item',
  'item.dynamicAttributes': 'Dynamic Attributes',
  'item.attributes.hpRestore': 'HP Restore',
  'item.attributes.hpRestoreHint': 'Amount of health restored',
  'item.attributes.vpRestore': 'VP Restore',
  'item.attributes.vpRestoreHint': 'Amount of vitality restored',
  'item.attributes.cooldown': 'Cooldown (seconds)',
  'item.attributes.cooldownHint': 'Time to wait after use',
  'item.attributes.faithCost': 'Faith Cost',
  'item.attributes.faithCostHint': 'Faith consumed when used',
  'item.attributes.deityId': 'Deity ID',
  'item.attributes.deityIdPlaceholder': 'Leave empty for generic',
  'item.attributes.deityIdHint': 'Corresponding deity ID (optional)',
  'item.attributes.crimeValue': 'Crime Value',
  'item.attributes.crimeValueHint': 'Crime value added when carrying',
  'item.attributes.policeDetection': 'Police Detection Multiplier',
  'item.attributes.policeDetectionHint': 'Police detection rate multiplier (1.0 = normal)',
  'item.attributes.stackLimit': 'Stack Limit',
  'item.attributes.stackLimitHint': 'Maximum stacks in single inventory slot',
  'item.imageUrl': 'Image',
  'item.missingId': 'Missing item ID',
  'item.loadFailed': 'Failed to load',
  'item.saveFailed': 'Failed to save',
  'item.saving': 'Saving...',
  'item.creating': 'Creating...',
  'item.saveChanges': 'Save Changes',
  'item.createItem': 'Create Item',
  'item.untitled': 'Untitled Item',
  'item.noDescription': 'No description',

  // Shop Management (Phase 16.3)
  'shop.title': 'Shop Management',
  'shop.subtitle': 'Manage all shops in the game',
  'shop.create': 'Create Shop',
  'shop.new': 'New Shop',
  'shop.newDescription': 'Create a new game shop',
  'shop.edit': 'Edit Shop',
  'shop.delete': 'Delete Shop',
  'shop.deleteConfirm': 'Confirm Delete',
  'shop.deleteCancel': 'Cancel',
  'shop.searchPlaceholder': 'Search shop name...',
  'shop.filterByStatus': 'Status',
  'shop.noShops': 'No shops created yet',
  'shop.noMatchingShops': 'No matching shops found',
  'shop.createFirst': 'Create your first shop',
  'shop.basicInfo': 'Basic Information',
  'shop.itemList': 'Item List',
  'shop.operatingHours': 'Operating Hours',
  'shop.name': 'Shop Name',
  'shop.namePlaceholder': 'e.g.: Temple Street Convenience Store',
  'shop.description': 'Shop Description',
  'shop.descriptionPlaceholder': 'Describe this shop...',
  'shop.isActive': 'Enable this shop',
  'shop.enableOperatingHours': 'Set operating hours (unchecked = 24 hours)',
  'shop.startTime': 'Start Time',
  'shop.endTime': 'End Time',
  'shop.crossMidnight': '⚠️ Cross-midnight mode (e.g., 22:00 - 04:00)',
  'shop.addItem': 'Add Item to Shop',
  'shop.selectItem': 'Select Item',
  'shop.removeItem': 'Remove Item',
  'shop.noItems': 'No items in this shop',
  'shop.itemStock': 'Stock',
  'shop.itemPersonalLimit': 'Personal Limit',
  'shop.itemPriceMultiplier': 'Price Multiplier',
  'shop.globalStock': 'Global Stock',
  'shop.globalStockHint': '-1 = unlimited, 0+ = limited stock',
  'shop.currentStock': 'Current Stock',
  'shop.personalLimit': 'Personal Limit (per player)',
  'shop.personalLimitHint': '0 = no limit, 1+ = limit per player',
  'shop.priceMultiplier': 'Price Multiplier',
  'shop.priceMultiplierHint': '1.0 = normal price, 1.2 = 20% markup',
  'shop.resetStock': 'Reset Stock',
  'shop.resetStockConfirm': 'Reset stock to initial values?',
  'shop.updateShop': 'Update Shop',
  'shop.createShop': 'Create Shop',
  'shop.backToList': 'Back to Shop List',
  'shop.loadFailed': 'Failed to load',
  'shop.notFound': 'Shop not found',
  'shop.missingId': 'Missing shop ID',
  'shop.saveFailed': 'Failed to save',
  'shop.saving': 'Saving...',
  'shop.creating': 'Creating...',
  'shop.defaultConfiguration': 'Default Configuration',
  'shop.configurationHint': 'These settings will apply to all selected items',
  'shop.selectedItems': 'Selected Items',
  'shop.selectItemsToAdd': 'Select Items to Add',
  'shop.availableItems': 'Available Items',
  'shop.allItemsAdded': 'All items already added',
  'shop.addSelectedItems': 'Add Selected Items',
  'shop.addedItems': 'Added Items',
  'shop.noItemsAdded': 'No items added yet',
  'shop.unlimited': 'Unlimited',
  'shop.stock': 'Stock',
  'shop.limit': 'Limit',

  // NPC Management
  'npc.title': 'NPC Management',
  'npc.subtitle': 'Manage all NPCs in the game',
  'npc.templates': 'Template Management',
  'npc.instances': 'Instance Management',
  'npc.templates.title': 'NPC Template Management',
  'npc.templates.subtitle': 'Manage NPC definitions and attributes',
  'npc.templates.create': 'Create Template',
  'npc.templates.edit': 'Edit Template',
  'npc.templates.delete': 'Delete Template',
  'npc.templates.deleteConfirm': 'Confirm Delete',
  'npc.templates.noTemplates': 'No NPC templates created yet',
  'npc.templates.noMatchingTemplates': 'No matching templates found',
  'npc.templates.createFirst': 'Create your first template',
  'npc.templates.searchPlaceholder': 'Search template name or model ID...',
  'npc.instances.title': 'NPC Instance Management',
  'npc.instances.subtitle': 'Place and manage NPC instances on the map',
  'npc.instances.create': 'Create Instance',
  'npc.instances.edit': 'Edit Instance',
  'npc.instances.delete': 'Delete Instance',
  'npc.instances.deleteConfirm': 'Confirm Delete',
  'npc.instances.noInstances': 'No NPC instances created yet',
  'npc.instances.noMatchingInstances': 'No matching instances found',
  'npc.instances.createFirst': 'Create your first instance',
  'npc.instances.searchPlaceholder': 'Search template name, map ID...',

  // NPC Type Labels
  'npc.type.CITIZEN': 'Citizen',
  'npc.type.POLICE': 'Police',
  'npc.type.GANGS': 'Gang Member',
  'npc.type.SHOP': 'Shop Owner',
  'npc.type.QUEST': 'Quest NPC',

  // NPC Combat Type Labels
  'npc.combatType.MELEE': 'Melee',
  'npc.combatType.RANGED': 'Ranged',

  // NPC Movement Pattern Labels
  'npc.movementPattern.STATIC': 'Static',
  'npc.movementPattern.WANDERING': 'Wandering',
  'npc.movementPattern.PATROLLING': 'Patrolling',

  // NPC Action Type Labels
  'npc.actionType.open_shop': 'Open Shop',
  'npc.actionType.accept_quest': 'Accept Quest',
  'npc.actionType.end_dialogue': 'End Dialogue',

  // NPC Dialogue Editor
  'npc.dialogueEditor.title': 'Dialogue Settings',
  'npc.dialogueEditor.addNode': 'Add Dialogue',
  'npc.dialogueEditor.content': 'Content',
  'npc.dialogueEditor.actionType': 'Action Type',
  'npc.dialogueEditor.playerOptions': 'Player Options',
  'npc.dialogueEditor.addOption': 'Add Option',
  'npc.dialogueEditor.saveTree': 'Save Dialogue',
  'npc.dialogueEditor.cancel': 'Cancel',
  'npc.dialogueEditor.optionText': 'Option Text',
  'npc.dialogueEditor.cannotDeleteLast': 'Cannot delete the last dialogue',
  'npc.dialogueEditor.newDialogueContent': 'New dialogue content',
  'npc.dialogueEditor.newOption': 'New option',
  'npc.dialogueEditor.defaultGreeting': 'Hello!',
  'npc.dialogueEditor.nodes': 'dialogues',
  'npc.dialogueEditor.delete': 'Delete',
  'npc.dialogueEditor.hint': 'Drag rows to reorder. Click expand arrow to edit action settings.',
  'npc.dialogueEditor.startNodeHint': 'Start dialogue',
  'npc.dialogueEditor.contentPlaceholder': 'Enter dialogue content...',
  'npc.dialogueEditor.actionNextDialogue': 'Next Dialogue',
  'npc.dialogueEditor.actionPlayerOptions': 'Player Options',
  'npc.dialogueEditor.actionOpenShop': 'Open Shop',
  'npc.dialogueEditor.actionEndDialogue': 'End Dialogue',
  'npc.dialogueEditor.useLinkedShop': 'Use NPC linked shop',
  'npc.dialogueEditor.confirmClearTree': 'Deleting the first dialogue will clear the entire dialogue tree. Are you sure?',
  'npc.dialogueEditor.deleteFirstNodeHint': 'Deleting the first dialogue will clear the entire dialogue tree',

  // NPC Template Form
  'npc.template.basicInfo': 'Basic Information',
  'npc.template.name': 'Template Name',
  'npc.template.namePlaceholder': 'e.g.: Friendly Citizen, Patrol Officer',
  'npc.template.type': 'NPC Type',
  'npc.template.modelId': 'Model ID',
  'npc.template.modelIdPlaceholder': 'e.g.: citizen_01, police_patrol',
  'npc.template.description': 'Description',
  'npc.template.descriptionPlaceholder': 'Describe the purpose and characteristics of this NPC template...',
  'npc.template.baseStats': 'Base Stats',
  'npc.template.hp': 'Health Points (HP)',
  'npc.template.attack': 'Attack',
  'npc.template.defense': 'Defense',
  'npc.template.speed': 'Speed',
  'npc.template.combatSettings': 'Combat Settings',
  'npc.template.combatType': 'Combat Type',
  'npc.template.attackRange': 'Attack Range',
  'npc.template.attackRangePlaceholder': 'Leave empty to use default',
  'npc.template.attackRangeNote': 'Combat type must be selected first',
  'npc.template.noCombat': 'No Combat',
  'npc.template.dialogueSettings': 'Dialogue Settings',
  'npc.template.noDialogue': 'No dialogue tree configured',
  'npc.template.noDialogueTree': 'No dialogue tree configured',
  'npc.template.hasDialogue': 'Dialogue tree configured with',
  'npc.template.dialogueTreeSet': 'Dialogue tree configured with',
  'npc.template.nodes': 'nodes',
  'npc.template.createDialogue': 'Create Dialogue Tree',
  'npc.template.createDialogueTree': 'Create Dialogue Tree',
  'npc.template.editDialogue': 'Edit Dialogue Tree',
  'npc.template.editDialogueTree': 'Edit Dialogue Tree',
  'npc.template.removeDialogue': 'Remove Dialogue Tree',
  'npc.template.removeDialogueTree': 'Remove Dialogue Tree',
  'npc.template.updateTemplate': 'Update Template',
  'npc.template.createTemplate': 'Create Template',
  'npc.template.backToList': 'Back to Template List',
  'npc.template.loadFailed': 'Failed to load',
  'npc.template.notFound': 'NPC template not found',
  'npc.template.missingId': 'Missing NPC template ID',
  'npc.template.editTitle': 'Edit NPC Template',
  'npc.template.editSubtitle': 'Modify NPC template settings',
  'npc.template.loading': 'Loading...',
  'npc.template.loadError': 'Failed to load NPC template',
  'npc.template.loadErrorTryAgain': 'Failed to load, please try again later',
  'npc.template.reload': 'Reload',

  // NPC Instance Form
  'npc.instance.templateSelection': 'NPC Template',
  'npc.instance.template': 'NPC Template',
  'npc.instance.selectTemplate': 'Select Template',
  'npc.instance.selectPlaceholder': 'Please select...',
  'npc.instance.level': 'Level',
  'npc.instance.position': 'Position & Placement',
  'npc.instance.positionPlacement': 'Position & Placement',
  'npc.instance.positionX': 'X Coordinate',
  'npc.instance.positionZ': 'Z Coordinate',
  'npc.instance.rotation': 'Rotation',
  'npc.instance.mapId': 'Map ID',
  'npc.instance.mapIdPlaceholder': 'e.g.: city_center',
  'npc.instance.territoryId': 'Territory ID',
  'npc.instance.territoryIdPlaceholder': 'e.g.: downtown',
  'npc.instance.interactionRadius': 'Interaction Radius',
  'npc.instance.useMapPicker': 'Use Visual Map Picker',
  'npc.instance.closeMap': 'Close Map',
  'npc.instance.visualMap': 'Visual Map',
  'npc.instance.movementSettings': 'Movement Settings',
  'npc.instance.movementPattern': 'Movement Pattern',
  'npc.instance.wanderRadius': 'Wander Radius',
  'npc.instance.wanderCenterX': 'Wander Center X',
  'npc.instance.wanderCenterZ': 'Wander Center Z',
  'npc.instance.patrolWaypoints': 'Patrol Waypoints',
  'npc.instance.addWaypoint': 'Add Waypoint',
  'npc.instance.combatSpecial': 'Combat & Special Settings',
  'npc.instance.combatSettings': 'Combat Settings',
  'npc.instance.aggroRange': 'Aggro Range',
  'npc.instance.chaseDistance': 'Chase Distance',
  'npc.instance.useDefault': 'Use default',
  'npc.instance.shopId': 'Shop ID',
  'npc.instance.shopIdPlaceholder': 'If this is a shop owner',
  'npc.instance.linkedShopId': 'Linked Shop',
  'npc.instance.linkedShopIdPlaceholder': 'Select a shop (Phase 16.3)',
  'npc.instance.isGuildOnly': 'Guild Only',
  'npc.instance.isGuildOnlyHint': 'Only guild members can access this NPC',
  'npc.instance.isAttackable': 'Attackable',
  'npc.instance.updateInstance': 'Update Instance',
  'npc.instance.createInstance': 'Create Instance',
  'npc.instance.backToList': 'Back to Instance List',
  'npc.instance.loadFailed': 'Failed to load',
  'npc.instance.notFound': 'NPC instance not found',
  'npc.instance.missingId': 'Missing NPC instance ID',
  'npc.instance.placeNew': 'Place a new NPC instance on the map',
  'npc.instance.editTitle': 'Edit NPC Instance',
  'npc.instance.editSubtitle': 'Modify NPC instance settings',
  'npc.instance.unknownTemplate': 'Unknown Template',
  'npc.instance.loading': 'Loading...',
  'npc.instance.loadError': 'Failed to load NPC instance',
  'npc.instance.loadErrorTryAgain': 'Failed to load, please try again later',
  'npc.instance.reload': 'Reload',

  // NPC Appearance Management (Phase 19)
  'npc.appearances': 'Appearance Management',
  'npc.appearances.title': 'NPC Appearance Management',
  'npc.appearances.subtitle': 'Customize NPC appearance with 3D character models',
  'npc.appearances.gender': 'Gender',
  'npc.appearances.male': 'Male',
  'npc.appearances.female': 'Female',
  'npc.appearances.hair': 'Hair',
  'npc.appearances.top': 'Top',
  'npc.appearances.bottom': 'Bottom',
  'npc.appearances.shoes': 'Shoes',
  'npc.appearances.beard': 'Beard',
  'npc.appearances.head': 'Head',
  'npc.appearances.hairColor': 'Hair Color',
  'npc.appearances.beardColor': 'Beard Color',
  'npc.appearances.none': 'None',
  'npc.appearances.hair.short': 'Short',
  'npc.appearances.hair.buzzed': 'Buzzed',
  'npc.appearances.hair.buzzedFemale': 'Buzzed (F)',
  'npc.appearances.hair.long': 'Long',
  'npc.appearances.hair.bun': 'Bun',
  'npc.appearances.beard.beard': 'Beard',
  'npc.appearances.head.cap': 'Cap',
  'npc.appearances.top.shirt01': 'Shirt',
  'npc.appearances.bottom.pants01': 'Pants',
  'npc.appearances.shoe.shoe01': 'Shoes',
  'npc.appearances.loadingModel': 'Loading 3D model...',
  'npc.appearances.loadError': 'Failed to load 3D model',
  'npc.appearances.rotateHint': 'Drag to rotate the model',

  // Dialogue Editor
  'dialogue.title': 'Dialogue Tree Editor',
  'dialogue.addNode': 'Add Node',
  'dialogue.startNode': 'Start Node',
  'dialogue.editNode': 'Edit Node',
  'dialogue.deleteNode': 'Cannot delete start node',
  'dialogue.speaker': 'Speaker',
  'dialogue.content': 'Dialogue Content',
  'dialogue.actionType': 'Action Type',
  'dialogue.actionData': 'Action Data (JSON)',
  'dialogue.actionDataPlaceholder': '{"shopId": "shop_001"}',
  'dialogue.noAction': 'No Action',
  'dialogue.playerOptions': 'Player Options',
  'dialogue.addOption': 'Add Option',
  'dialogue.optionText': 'Option Text',
  'dialogue.saveTree': 'Save Dialogue Tree',
  'dialogue.saving': 'Saving...',

  // Map Position Picker
  'map.zoomIn': 'Zoom In',
  'map.zoomOut': 'Zoom Out',
  'map.resetView': 'Reset View',
  'map.rotation': 'Rotation',
  'map.instructions': 'Instructions',
  'map.instruction1': 'Click on the map to set NPC position',
  'map.instruction2': 'Use middle or right mouse button to drag and pan',
  'map.instruction3': 'Use zoom buttons to adjust view size',
  'map.instruction4': 'Green dot shows current position, arrow shows facing direction',
  'map.coordinateX': 'X Coordinate',
  'map.coordinateZ': 'Z Coordinate',
  'map.rotationAngle': 'Rotation Angle',

  // Errors and Messages
  'error.loadFailed': 'Failed to load',
  'error.saveFailed': 'Failed to save',
  'error.deleteFailed': 'Failed to delete',
  'error.unknownError': 'An unknown error occurred',
  'error.networkError': 'Network error',
  'error.tryAgain': 'Please try again later',
  'error.reload': 'Reload',
  'error.required': 'This field is required',
  'error.invalidValue': 'Invalid value',
  'error.mustBePositive': 'Must be greater than 0',
  'error.mustBeNonNegative': 'Cannot be negative',
  'error.minWaypoints': 'Patrol mode requires at least 2 waypoints',
  'error.wanderRadiusRequired': 'Wander mode requires wander radius',
  'error.firebaseConnection': 'Firebase connection failed. Please check environment variables.',
  'error.blockedByClient': 'Request blocked by browser. Please check ad blockers or firewall extensions.',
  'error.permissionDenied': 'Firebase permission denied. Please check Firestore security rules.',
  'error.serviceUnavailable': 'Firebase service temporarily unavailable. Please try again later.',

  // Success Messages
  'success.saved': 'Saved successfully',
  'success.created': 'Created successfully',
  'success.updated': 'Updated successfully',
  'success.deleted': 'Deleted successfully',

  // Empty States
  'empty.noData': 'No data available',
  'empty.noResults': 'No results found',
  'empty.startCreating': 'Start creating your first item',

  // Quest Blueprint System (Phase 20)
  'quest.title': 'Quest Management',
  'quest.subtitle': 'Design and manage quest blueprints with visual node editor',
  'quest.create': 'Create Quest',
  'quest.searchPlaceholder': 'Search quest name or description...',
  'quest.deleteConfirm': 'Are you sure you want to delete this quest',
  'quest.noBlueprints': 'No quest blueprints created yet',
  'quest.noMatchingBlueprints': 'No matching quest blueprints found',
  'quest.createFirst': 'Create your first quest blueprint',
  'quest.nodeCount': 'Nodes',
  'quest.edgeCount': 'Connections',
  'quest.updatedAt': 'Updated',
  'quest.backToList': 'Back to Quest List',
  'quest.namePlaceholder': 'Enter quest name...',
  'quest.descriptionPlaceholder': 'Quest description (optional)',
  'quest.nameRequired': 'Quest name is required',
  'quest.saving': 'Saving...',
  'quest.updateBlueprint': 'Update Quest',
  'quest.createBlueprint': 'Create Quest',
  'quest.notFound': 'Quest blueprint not found',
  'home.questManagement': 'Quest Management',
  'home.questManagement.desc': 'Design quest flows with visual node editor',
  'home.mapEditor.desc': 'Move, replace and add map buildings in a 3D scene',
  'home.mapAssets.desc': 'Upload and manage building models for the map',
  'home.userAccounts.desc': 'Manage dashboard accounts: groups and active status',
  'home.userGroups.desc': 'Set permissions per group, then add accounts to groups',
  'home.noAccess': 'You have no accessible modules yet. Please ask an admin to assign permissions.',

  // Quest Node Types
  'quest.node.start': 'Start',
  'quest.node.dialogue': 'Dialogue',
  'quest.node.choice': 'Choice',
  'quest.node.task': 'Task',
  'quest.node.condition': 'Condition',
  'quest.node.action': 'Action',
  'quest.node.end': 'End',

  // Start Node
  'quest.node.npcTemplateId': 'NPC Template ID',
  'quest.node.npcTemplateIdPlaceholder': 'e.g.: quest_giver_01',
  'quest.node.minLevel': 'Min Level',
  'quest.node.maxLevel': 'Max Level',
  'quest.node.prerequisiteQuestId': 'Prerequisite Quest',
  'quest.node.prerequisiteQuestIdPlaceholder': 'Quest ID required to complete first',

  // Dialogue Node
  'quest.node.speakerId': 'Speaker ID',
  'quest.node.expression': 'Expression',
  'quest.node.expressionPlaceholder': 'e.g.: happy, angry',
  'quest.node.textZh': 'Text (zh-TW)',
  'quest.node.textZhPlaceholder': 'Enter Chinese dialogue text...',
  'quest.node.textEn': 'Text (EN)',
  'quest.node.textEnPlaceholder': 'Enter English dialogue text...',

  // Choice Node
  'quest.node.option': 'Option',
  'quest.node.optionTextZh': 'Option text (zh-TW)',
  'quest.node.optionTextEn': 'Option text (EN)',
  'quest.node.addOption': 'Add Option',

  // Task Node
  'quest.node.taskType': 'Task Type',
  'quest.node.targetId': 'Target ID',
  'quest.node.targetIdPlaceholder': 'Item / Enemy / NPC ID',
  'quest.node.requiredCount': 'Required Count',
  'quest.node.radius': 'Radius',
  'quest.taskType.collect': 'Collect Items',
  'quest.taskType.kill': 'Kill Enemies',
  'quest.taskType.interact': 'Talk to NPC',
  'quest.taskType.location': 'Reach Location',

  // Condition Node
  'quest.node.conditionType': 'Condition Type',
  'quest.node.requiredAmount': 'Required Amount',
  'quest.node.success': 'Success',
  'quest.node.fail': 'Fail',
  'quest.conditionType.money': 'Money Check',
  'quest.conditionType.item': 'Item Check',
  'quest.conditionType.variable': 'Variable Check',

  // Action Node
  'quest.node.actionType': 'Action Type',
  'quest.node.amount': 'Amount',
  'quest.node.value': 'Value',
  'quest.actionType.removeItem': 'Remove Item',
  'quest.actionType.removeMoney': 'Remove Money',
  'quest.actionType.spawnNpc': 'Spawn NPC',
  'quest.actionType.setVariable': 'Set Variable',

  // End Node
  'quest.node.rewardXp': 'Reward XP',
  'quest.node.rewardMoney': 'Reward HKD',
  'quest.node.rewardItems': 'Reward Items',
  'quest.node.itemId': 'Item ID',
  'quest.node.addRewardItem': 'Add Reward Item',
  'quest.deleteEdgeConfirm': 'Delete this connection?',
  'quest.deleteNodeConfirm': 'Delete this node?',
  'quest.emoji.faces': 'Faces',
  'quest.emoji.gestures': 'Gestures',
  'quest.emoji.symbols': 'Symbols',
  'quest.emoji.objects': 'Objects',
  'quest.node.variableName': 'Variable Name',
  'quest.node.variableNamePlaceholder': 'e.g. talked_to_boss',
  'quest.node.variableHint': 'Custom variable set via Action node (set_variable)',

  // Map Editor
  'map.editor.title': 'Map Editor',
  'map.editor.subtitle': 'Load the map, inspect and select buildings and objects',
  'map.editor.selectChunk': 'Select map chunk',
  'map.editor.loadingManifest': 'Loading map list...',
  'map.editor.loadingMap': 'Loading map...',
  'map.editor.manifestError': 'Failed to load map list',
  'map.editor.mapError': 'Failed to load map model',
  'map.editor.checkSource': 'Check the map source URL (NEXT_PUBLIC_MAP_BASE_URL) and make sure the server hosting the maps is running.',
  'map.editor.help': 'Left-drag to orbit, scroll to zoom, right-drag to pan; click to select, double-click to zoom to a model.',
  'map.editor.noObjectSelected': 'No object selected',
  'map.editor.noObjectHint': 'Click a building or object on the map to view its details.',
  'map.inspector.title': 'Object Info',
  'map.inspector.name': 'Name',
  'map.inspector.type': 'Type',
  'map.inspector.chunk': 'Chunk',
  'map.inspector.key': 'Key',
  'map.inspector.position': 'Position (chunk-local)',
  'map.inspector.rotation': 'Rotation (radians)',
  'map.inspector.rotationDeg': 'Rotation (deg)',
  'map.inspector.scale': 'Scale',
  'map.inspector.size': 'Size',
  'map.objectType.building': 'Building',
  'map.objectType.prop': 'Prop',
  'map.objectType.terrain': 'Terrain',
  'map.objectType.other': 'Other',
  'map.editor.gizmoMode': 'Transform mode',
  'map.editor.gizmo.move': 'Move',
  'map.editor.gizmo.rotate': 'Rotate',
  'map.editor.gizmo.scale': 'Scale',
  'map.editor.dragHint': 'Drag the handles in the 3D scene, then click "Save changes".',
  'map.editor.save': 'Save changes',
  'map.editor.saving': 'Saving...',
  'map.editor.saveFailed': 'Save failed, please try again.',
  'map.editor.remove': 'Remove',
  'map.editor.removeConfirm': 'Confirm remove',
  'map.editor.reset': 'Reset to original',
  'map.editor.restore': 'Restore',
  'map.editor.unsaved': 'You have unsaved changes',
  'map.editor.removedHint': 'This object is removed and will not appear in-game; click "Restore" to undo.',
  'map.status.original': 'Original',
  'map.status.modified': 'Modified',
  'map.status.removed': 'Removed',
  'map.status.replaced': 'Replaced',
  'map.status.added': 'Added',
  'map.status.disabled': 'Disabled',
  'map.editor.enable': 'Enable',
  'map.editor.disable': 'Disable',
  'map.editor.disabledHint': 'This edit is disabled and won\'t apply in-game; click "Enable" to restore.',
  'map.editor.readOnly': 'Read-only: you do not have permission to edit the map.',
  'map.editor.replace': 'Replace with asset',
  'map.editor.changeAsset': 'Change asset',
  'map.editor.addBuilding': 'Add building',
  'map.editor.deleteBuilding': 'Delete this building',
  'map.editor.publish': 'Publish',
  'map.editor.publishing': 'Publishing...',
  'map.editor.publishConfirm': 'Publish all active map edits as a snapshot? Players will load this snapshot instead of querying overrides chunk by chunk.',
  'map.editor.publishSuccess': 'Published {count} edit(s){chunked}.',
  'map.editor.publishSuccessChunkedSuffix': ' (split into multiple parts, over 900KB)',
  'map.editor.publishFailed': 'Publish failed, please try again.',
  'map.group.defaultName': 'Group',
  'map.group.create': 'Create group',
  'map.group.ungroup': 'Remove from group',
  'map.group.dissolve': 'Dissolve group',
  'map.group.rename': 'Rename',
  'map.group.selected': '{count} selected',
  'map.group.clearSelection': 'Clear selection',
  'map.group.selectHint': 'Select to add to a group',
  'map.group.notGroupable':
    'Only objects replaced or added from the asset library can be grouped',
  'map.list.selectHint': 'Ctrl to add · Shift for range · double-click to focus',
  'map.list.loadingInstances': 'Loading {count} asset instance(s)…',
  'map.list.orphanAsset':
    'These edits reference an asset that was deleted from the library. They can never render, and the spot will be blank in game:',
  'map.list.cleanOrphans': 'Delete these {count} stale edit(s)',
  'map.editor.groupId': 'Occlusion group',
  'map.editor.groupIdPlaceholder': 'Empty = its own group',
  'map.editor.groupIdHint':
    'Objects sharing a value are treated as one building and fade out together when the player walks behind them. Use this when the ground floor and upper floors are placed as separate assets.',
  'map.list.title': 'Map Objects',
  'map.list.empty': 'No selectable objects in this chunk',
  'map.list.dblClickHint': 'Click to select, double-click to zoom in',
  'map.list.showRemoved': 'Show removed',
  'map.list.hideRemoved': 'Hide removed',
  'map.tab.inspector': 'Selected',
  'map.tab.list': 'Objects',
  'map.list.loadFailed': 'These edits are saved but not shown in the scene (possible model or coordinate issue):',
  'map.assets.title': 'Building Asset Library',
  'map.assets.subtitle': 'Upload building GLBs to replace or add buildings in the map editor',
  'map.assets.upload': 'Upload building',
  'map.assets.uploadTitle': 'Upload new building',
  'map.assets.file': 'GLB file',
  'map.assets.name': 'Name',
  'map.assets.deleteInUse':
    'Cannot delete: this asset is still used in {count} place(s) on the map ({chunks}). Remove them in the map editor first, otherwise those spots will be blank in game.',
  'map.assets.kind': 'Usage',
  'map.assets.kind.building': 'Building',
  'map.assets.kind.prop': 'Street prop',
  'map.assets.kind.decal': 'Ground decal',
  'map.assets.kindHint.building':
    'Solid building: has collision and fades out when the player walks behind it.',
  'map.assets.kindHint.prop':
    'Street prop: has collision but never fades out from occlusion.',
  'map.assets.kindHint.decal':
    'Ground decal (road arrows, crossings, manhole covers): no collision, not clickable, and depth-offset so it will not z-fight with the road surface.',
  'map.assets.category': 'Category',
  'map.assets.defaultScale': 'Default scale',
  'map.assets.tags': 'Tags',
  'map.assets.tagsHint': 'Comma-separated',
  'map.assets.submit': 'Upload',
  'map.assets.uploading': 'Uploading...',
  'map.assets.generatingThumb': 'Generating thumbnail...',
  'map.assets.empty': 'No building assets yet',
  'map.assets.uploadFirst': 'Upload your first building',
  'map.assets.edit': 'Edit building',
  'map.assets.save': 'Save',
  'map.assets.deleteConfirm': 'Confirm delete',
  'map.assets.uploadFailed': 'Upload failed, please try again.',
  'map.assets.loadFailed': 'Failed to load building assets',
  'map.assets.invalidFile': 'Please select a .glb file',
  'map.assets.tooLarge': 'File too large (10MB limit on the free plan); use a smaller model.',

  // Auth
  'auth.title': 'Admin Sign In',
  'auth.subtitle': 'Please sign in as an admin to continue',
  'auth.signInGoogle': 'Sign in with Google',
  'auth.signOut': 'Sign out',
  'auth.pending': 'Account pending',
  'auth.pendingHint': 'Your account is not yet authorized or has been disabled. Please ask an admin to assign permissions.',
  'auth.noPermission': 'No admin permission',
  'auth.noPermissionHint': 'This account is not on the admin list. Please sign in with an admin account.',
  'auth.switchAccount': 'Switch account',
  'auth.allowlistWarning': 'No admin allowlist set — any signed-in account can access. Create an "admins" collection in Firestore (doc ID = admin email), or set NEXT_PUBLIC_ADMIN_EMAILS.',

  // User management
  'users.accounts.title': 'Account Management',
  'users.accounts.subtitle': 'Manage dashboard accounts: groups and active status',
  'users.accounts.loadFailed': 'Failed to load accounts',
  'users.accounts.pending': 'Pending',
  'users.accounts.noGroups': 'No groups assigned',
  'users.accounts.assignGroups': 'Assign groups',
  'users.accounts.noGroupsAvailable': 'No groups yet — create one under "Groups" first.',
  'users.accounts.cannotSelf': 'You cannot disable your own account',
  'users.groups.title': 'Group Management',
  'users.groups.subtitle': 'Set permissions per group, then add accounts to groups',
  'users.groups.loadFailed': 'Failed to load groups',
  'users.groups.create': 'New group',
  'users.groups.edit': 'Edit group',
  'users.groups.empty': 'No groups yet',
  'users.groups.name': 'Name',
  'users.groups.description': 'Description',
  'users.groups.permissions': 'Permissions',
  'users.groups.module': 'Module',
  'users.perm.view': 'View',
  'users.perm.edit': 'Edit',
  'users.groups.active': 'Enable this group',
  'users.groups.noPermissions': 'No permissions',

  // Validation
  'validation.required': 'Please enter {field}',
  'validation.mustBePositive': '{field} must be greater than 0',
  'validation.mustBeNonNegative': '{field} cannot be negative',
  'validation.invalidFormat': '{field} format is incorrect',
} as const;
