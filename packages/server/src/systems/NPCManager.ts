...
export class NPCManager {
    ...
    /**
     * 從 NPC 定義資料生成 NPC (Phase 16-2: 支援自定義模型)
     */
    private spawnNPCFromData(data: INPCData): void {
        ...
        console.log(`✅ [NPCManager] Spawned ${data.type} NPC "${data.name}" at (${npc.x}, ${npc.z})${data.modelId ? ` with model: ${data.modelId}` : ' with default model'}`);
    }
